...
class QRSDetector(object):
    ...
    def __init__(
        self, ecg_data, verbose=False, log_data=False, plot_data=False, show_plot=False
    ):
        """
        ...
        self.signal_frequency = (
            250  # Set ECG device frequency in samples per second here.
        )
        ...
        self.integration_window = (
            15  # Change proportionally when adjusting frequency (in samples).
        )

        self.findpeaks_limit = 0.35
        self.findpeaks_spacing = (
            50  # Change proportionally when adjusting frequency (in samples).
        )

        self.refractory_period = (
            120  # Change proportionally when adjusting frequency (in samples).
        )
    ...
    def detect_peaks(self):
        ...
        self.detected_peaks_values = self.integrated_ecg_measurements[
            self.detected_peaks_indices
        ]
    ...
    def detect_qrs(self):
        ...
        for detected_peak_index, detected_peaks_value in zip(
            ...
            if (
                detected_peak_index - last_qrs_index > self.refractory_period
                or not self.qrs_peaks_indices.size
            ):
                ...
                if detected_peaks_value > self.threshold_value:
                    ...
                    self.qrs_peak_value = (
                        self.qrs_peak_filtering_factor * detected_peaks_value
                    ...
                    self.noise_peak_value = (
                        self.noise_peak_filtering_factor * detected_peaks_value
    ...
    def plot_detection_data(self, show_plot=False):
        ...
        plot_data(
            axis=axarr[0], data=self.ecg_data_raw[:, 1], title="Raw ECG measurements"
        )
        plot_data(
            axis=axarr[1],
        ...
        plot_points(
            axis=axarr[5],
            values=self.ecg_data_detected[:, 1],
            indices=self.qrs_peaks_indices,
        )
    ...
    def bandpass_filter(self, data, lowcut, highcut, signal_freq, filter_order):
        ...
        b, a = butter(
            filter_order, fs=signal_freq, Wn=[lowcut, highcut], btype="bandpass"
        )
        y = lfilter(b, a, data)
        return y
...
if __name__ == "__main__":
    ...
    i = 2
    data = np.loadtxt(cwd + "/ecg_data_1.csv", skiprows=1, delimiter=",")[
        800 * i : 800 * (i + 1) + 100, :
    ]
